...
def seeding_number(
    ...
    if target_cells >= carrying_capacity:
        # Asking for ≥  K means we need to seed \u8805  K (logistic asymptotes to K)
...
with title_col:
    st.title("🧫 Cell Culture Seeding Calculator")
    st.caption(
    ...
    for hours in timepoints_to_use:
        ...
        for vessel_name in selected_vessels:
            ...
            if seeded_raw < floor:
                ...
                data[col_name].append(f"≥ {format_cells(round_nice(floor))}*")
...
with st.expander("How this works"):
    st.markdown(
        ...
        **The math: logistic growth**
        ...
        - `N₀ ` \'97 what we want to find

        Inverted to solve for the seeding number:

        ```
        N₀  = K \'b7 N / (N + (K - N) \'b7 e^(r\'b7t))
        ```
        ...
        **The practical floor**
        ...
        1/√ N. Below ~50 cells/well, CV climbs above 14% and replicates
        get inconsistent regardless of the math.

        The app uses `floor = max(50, 0.001 \'d7 K) / pe` and shows `≥ floor*`
        whenever the math is below it.

        **Why short and long timepoints behave differently**

        - **Short timepoints**: cells are far from confluence, logistic ≈ 
